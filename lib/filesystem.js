...
function getChcpEncoding(){
  let stdout = execSync('chcp',opts)
  var codePage = stdout.replace(/\D/g, '');
  return codePage && codePage !== '65001' ? 'cp' + codePage : 'utf8';
}
    ...
    process.nextTick(() => {
      ...
      if (_windows) {
        ...
        exec(util.getWmic() + ' logicaldisk get Caption,Description,DeviceID,DriveType,FileSystem,FreeSpace,Name,Size,VolumeName,VolumeSerialNumber /value', opts, function (error, stdout) {
          if (!error) {
            let devices = iconv.decode(stdout,getChcpEncoding()).split(/\n\s*\n/);
